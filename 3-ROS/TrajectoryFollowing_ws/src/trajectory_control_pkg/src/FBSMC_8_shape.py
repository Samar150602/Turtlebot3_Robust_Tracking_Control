...
# Differential Flatness Based Sliding Mode Tracking Control
...
gain_x = 2 # 3
gain_y = 2 # 3
gain_dis_x = 0.015 # 0.015 # 0.001
gain_dis_y = 0.015 # 0.015 # 0.001
...
once = True
file_path = '/home/esirem/Desktop/M2_Learning/3-M2_ViBot/4-RTIC/2-Project/3-ROS/TrajectoryFollowing_ws/src/trajectory_control_pkg/robot_data_csv_files/FBSMC_8shape_data.csv'
...
def figure_eight_trajectory(A, B, omega):
    ...
    while not rospy.is_shutdown():
        ...
        if x_positions and y_positions:  # Ensure there's data available
            ...
            s_x = derivative_error_x - (gain_x * error_x)
            s_y = derivative_error_y - (gain_y * error_y)
            
            u_eq_x = x_target_double_dot + (gain_x * derivative_error_x)
            u_eq_y = y_target_double_dot + (gain_y * derivative_error_y)
            
            u_dis_x = -gain_dis_x * math.tanh(s_x/0.1)
            u_dis_y = -gain_dis_y * math.tanh(s_y/0.1)
            
            # FBSMC control for v1 and v2
            v1 = u_eq_x + u_dis_x
            v2 = u_eq_y + u_dis_y
            ...
            max_linear_speed = 0.5  # Set maximum linear speed
            max_angular_speed = 1.0  # Set maximum angular speed
...
def plot_trajectory():
    ...
    plt.title("8-Shape Trajectory Tracking with FBSMC")
    plt.legend()
    plt.grid()
    plt.show()
...
if __name__ == '__main__':
    try:
        ...
        figure_eight_trajectory(A=1, B=-1, omega=0.04) # A=1, B=1, omega=0.1465
